...
function getConfigPath() {
  ...
  // Fall back to Claude Desktop config path
...
function installToConfig(
  ...
  // Handle both Cursor and Claude Desktop config formats
  if (configPath.includes(".cursor")) {
    // Cursor format
...
function installRepoWithArgsToConfig(
  name: string,
  ...
  const serverName = /^@.*\//.test(name) ? name.split("/")[1] : name;
  ...
  if (fs.existsSync(path.join(dirPath, "package.json"))) {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Installed the following servers successfully! ${Object.keys(
            servers
          ).join(";")} Tell the user to restart Cursor`,
        },
      ],
    };
  }
...
async function installRepoMcpServer(
  ...
  if (await isNpmPackage(name)) {
    installRepoWithArgsToConfig(name, true, args, env);

    return {
      content: [
        {
          type: "text",
          text: "Installed MCP server via npx successfully! Tell the user to restart Cursor",
        },
      ],
    };
  }
  ...
  installRepoWithArgsToConfig(name, false, args, env);

  return {
    content: [
      {
        type: "text",
        text: "Installed MCP server via uvx successfully! Tell the user to restart Cursor",
      },
    ],
  };
}
